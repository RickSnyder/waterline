...
Connections.prototype._build = function _build(adapters, options) {
  ...
  Object.keys(options).forEach(function(key) {
    ...
    if(!hasOwnProperty(config, 'module')) {
      msg = 'A connection must have a module attribute that describes which adapter to use';
      throw new Error(msg);
    }

    // Ensure the adapter exists in the adapters options
    if(!hasOwnProperty(adapters, config.module)) {
      msg = 'Adapter specified is not available in the options. You specified ' + config.module + '.';
      throw new Error(msg);
    }

    // Build the connection config
    connection = _.cloneDeep(config);
    connection._adapter = _.cloneDeep(adapters[config.module]);
    connection._collections = [];
