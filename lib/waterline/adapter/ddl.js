...
module.exports = {
  ...
  define: function(cb) {
    ...
    this.describe(function(err, existingAttributes) {
      ...
      if(!hasOwnProperty(self.dictionary, 'define')) return cb(new Error(errMsg));
  ...
  describe: function(cb) {
    ...
    // Find the connection to run this on
    if(!hasOwnProperty(this.dictionary, 'describe')) return cb(new Error(err));
  ...
  alter: function(cb) {
    ...
    if(this.adapter.alter) {
      return this.adapter.alter(schema, cb);
    }
