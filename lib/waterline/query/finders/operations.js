...
Operations.prototype._stageOperations = function _stageOperations(connections) {
  ...
  Object.keys(connections).forEach(function(connection) {
    ...
    var operation = {
      ...
      collection: connections[connection].joins[0].child,
      method: 'find',
      join: connections[connection].joins[0]
    };

    if(connections[connection].joins.length > 1) {
      operation.child = {
        connection: connection,
        collection: connections[connection].joins[1].child,
        method: 'find',
        join: connections[connection].joins[1]
      };
    }
...
Operations.prototype._createParentOperation = function _createParentOperation(connections) {
  ...
  var connectionName = this.context.adapterDictionary[this.parent];
...
Operations.prototype._runChildOperations = function _runChildOperations(intermediateResults, opt, cb) {
  ...
  res.forEach(function(result) {
    parents.push(result[opt.join.child.parentKey]);
  });
